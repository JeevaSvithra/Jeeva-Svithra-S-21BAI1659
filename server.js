...
function handleMove(player, message) {
  try {
    ...
    if (target) {
      ...
      broadcastCaptureMessage(target);
    }
    
    function broadcastCaptureMessage(character) {
      const captureMessage = JSON.stringify({ type: "capture", character });
      players.forEach(player => player.send(captureMessage));
    } 
    ...
    function checkWinCondition() {
      const aPieces = gameState.board.flat().filter(cell => cell && cell.startsWith("A"));
      const bPieces = gameState.board.flat().filter(cell => cell && cell.startsWith("B"));
    ...
    function broadcastWin(winner) {
      const winMessage = JSON.stringify({ type: "WIN", winner });
      players.forEach((player) => player.send(winMessage));
    }
    
    // Call checkWinCondition after a move
    function handleMove(player, message) {
      try {
        // Existing logic for handling moves...
