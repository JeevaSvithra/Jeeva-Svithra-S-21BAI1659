...
function updateSelectedMove(character, move) {
  ...
  selectedMove.textContent = `Selected Earlier: ${character} - ${move}`;
}
...
socket.onmessage = function (event) {
  try {
    ...
    if (message.type === "STATE") {
      ...
      showCaptureToast(message.character);
    }
  } catch (error) {
    console.error("Error parsing WebSocket message:", error);
  }
...
// Show capture toast
function showCaptureToast(character) {
  const toastBody = document.getElementById("toastBody");
  toastBody.textContent = `Character ${character} was captured!`;
